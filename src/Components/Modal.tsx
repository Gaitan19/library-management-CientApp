...
export default function LibraryManagement() {
  ...
  const fetchBooks = async () => {
    try {
      const res = await axios.get<Book[]>("http://localhost:5015/api/Books");
      setBooks(res.data);
    } catch (error) {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get<User[]>("http://localhost:5015/api/Users");
      setUsers(res.data);
    } catch (error) {
  ...
  const handleAdd = async () => {
    try {
      if (activeTab === "books") {
        await axios.post("http://localhost:5015/api/Books", newData);
      } else {
        await axios.post("http://localhost:5015/api/Users", newData);
      }
      toast.success(
  ...
  const handleBorrow = async (userId: string) => {
    ...
    try {
      await axios.post("http://localhost:5015/borrow", { userId, bookId });
  ...
  const handleReturn = async (userId: string, bookId: string) => {
    try {
      await axios.post("http://localhost:5015/return", { userId, bookId });
